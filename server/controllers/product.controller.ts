...
const productController = {
  ...
  createProduct: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(201)
        .json({ message: `${product.sku} created successfully`, product });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === "P2002") {
          return res
            .status(400)
            .json({ error: "Product already includes that component" });
        }
        if (error.code === "P2003" || error.code === "P2025") {
          return res
            ...
            .json({
              error:
                "One or more referenced parts, hardware, or hardware kits do not exist",
            });
        }
      }
      res.status(500).json({ error: "Internal Server Error" });
  ...
  updateProduct: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: `${product.sku} updated successfully`, product });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === "P2025") {
          return res.status(404).json({ error: "Product not found" });
        ...
        if (error.code === "P2002") {
          return res
            .status(400)
            .json({ error: "Another product already uses that sku" });
        }
      }
      res.status(500).json({ error: "Internal Server Error" });
    }
  ...
  archiveProduct: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: `${product.sku} archived successfully`, product });
    } catch (error) {
      res.status(500).json({ error: "Internal Server Error" });
    }
  },
  ...
  unarchiveProduct: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: `${product.sku} unarchived successfully`, product });
    } catch (error) {
      res.status(500).json({ error: "Internal Server Error" });
    }
  },
