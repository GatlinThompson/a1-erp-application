...
export default function EditProductPage() {
  ...
  const handleLoad = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/products/${encodeURIComponent(lookupSku.trim())}`);
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-6 py-10">
      ...
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        Updates <code>{API_URL}/products/:sku</code>. Saving replaces each of parts/hardware/
        hardware kits with exactly what&apos;s shown below — add, edit quantities, or remove
        rows, then save.
      </p>
